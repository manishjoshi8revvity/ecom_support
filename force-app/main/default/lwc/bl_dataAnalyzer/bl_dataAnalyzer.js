...
import BL_Log_Analyzer from "@salesforce/resourceUrl/BL_Log_Analyzer";
import getCustomerNames from "@salesforce/apex/BL_PunchoutLogsAnalyzerController.getCustomerNames";
import getCurrencyValues from "@salesforce/apex/BL_PunchoutLogsAnalyzerController.getCurrencyValues";
import getPunchoutLogs from "@salesforce/apex/BL_PunchoutLogsAnalyzerController.getPunchoutLogs";
import getPunchoutLogsByCustomer from "@salesforce/apex/BL_PunchoutLogsAnalyzerController.getPunchoutLogsByCustomer";

export default class Bl_dataAnalyzer extends LightningElement {
    ...
    Promise.all([
      ...
      .then((results) => {
        this.customerList = results[0]
          ? results[0].map((ar) => ar.Customer_Name__c)
          : [];

        this.currencyList = results[1]
          ? results[1].map((ar) => ar.Order_Total_Currency__c)
          : [];
      })
      .catch((error) => {
  ...
  renderedCallback() {
    if (!this.thirdPartyLibLoaded) {
      ...
      Promise.all([
        loadScript(this, BL_Log_Analyzer + "/chart.min.js"),
        loadScript(this, BL_Log_Analyzer + "/bootstrap.min.js"),
        loadStyle(this, BL_Log_Analyzer + "/bootstrap.min.css"),
        loadScript(this, BL_Log_Analyzer + "/luxon.min.js"),
        loadScript(this, BL_Log_Analyzer + "/tabulator.min.js"),
        loadStyle(this, BL_Log_Analyzer + "/tabulator_bootstrap5.min.css")
      ])
        .then(async () => {
          await loadScript(
            this,
            BL_Log_Analyzer + "/chartjs-adapter-luxon.min.js"
          );
  ...
  async handleSubmit() {
    try {
      ...
      result1.forEach((record) => {
        ...
        if (record.Order_Total_Currency__c) {
          currencies.add(record.Order_Total_Currency__c);

          orderDataMap[key + "-" + record.Order_Total_Currency__c] = {
      ...
      result2.forEach((record) => {
        let customerName = record.Customer_Name__c;
        if (
          !customerName ||
          (record.Order_Total_Currency__c && !record.poomCount)
        ) {
          return;
        }

        let currency = record.Order_Total_Currency__c;
